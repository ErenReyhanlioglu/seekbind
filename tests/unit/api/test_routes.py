...
async def test_health_check_returns_healthy_when_all_dependencies_healthy(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        routes_module,
        "check_postgres",
        AsyncMock(return_value=DependencyStatus(name="postgres", healthy=True)),
    )
    monkeypatch.setattr(
        routes_module,
        "check_qdrant",
        AsyncMock(return_value=DependencyStatus(name="qdrant", healthy=True)),
    )
    monkeypatch.setattr(
        routes_module,
        "check_llm_config",
        MagicMock(return_value=DependencyStatus(name="llm_config", healthy=True)),
    )
...
async def test_health_check_returns_unhealthy_when_any_dependency_unhealthy(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "check_postgres",
        AsyncMock(
            return_value=DependencyStatus(name="postgres", healthy=False, detail="hata")
        ),
    )
    monkeypatch.setattr(
        routes_module,
...
async def test_recommend_calls_get_recommendation_with_request_fields(
    monkeypatch,
) -> None:
    captured: dict = {}
    ...
    async def fake_get_recommendation(**kwargs):
        ...
        return RecommendationResponse(
            recommendation="test önerisi", results=[], total=0
        )
