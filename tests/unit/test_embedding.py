...
def test_ollama_embedding_name_derives_from_sanitized_model_tag(
    ...
    fake_settings = real_settings.model_copy(
        update={"ollama_embedding_model": "alibayram/embeddingmagibu-200m"}
    )
...
def test_ollama_embedding_dimension_matches_known_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    real_settings = embedding_module.get_settings()
    fake_settings = real_settings.model_copy(
        update={"ollama_embedding_model": "qwen3-embedding:0.6b"}
    )
    monkeypatch.setattr(embedding_module, "get_settings", lambda: fake_settings)
...
def test_ollama_embedding_raises_value_error_for_unknown_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    real_settings = embedding_module.get_settings()
    fake_settings = real_settings.model_copy(
        update={"ollama_embedding_model": "bilinmeyen-model:1b"}
    )
    monkeypatch.setattr(embedding_module, "get_settings", lambda: fake_settings)
