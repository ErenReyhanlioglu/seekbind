...
async def test_book_succeeds_for_free_slot_without_conflicts(
    ...
    booking = (
        await savepoint_session.execute(
            select(Booking).where(Booking.appointment_slot_id == slot.id)
        )
    ).scalar_one()
...
async def test_book_returns_alternatives_when_slot_already_booked(
    ...
    target_slot = await _create_slot(
        savepoint_session, target_business.id, _future_start_time(), is_booked=True
    )
    user = await _create_user(savepoint_session)
    ...
    await _create_slot(
        savepoint_session,
        alternative_business.id,
        _future_start_time(hour=14),
        is_booked=False,
    )
    ...
    assert any(
        alt["business"]["id"] == alternative_business.id for alt in body["alternatives"]
    )
...
async def test_book_returns_alternatives_when_user_has_conflicting_appointment(
    ...
    existing_slot = await _create_slot(
        savepoint_session, existing_business.id, conflicting_start, is_booked=True
    )
    savepoint_session.add(
        Booking(user_id=user.id, appointment_slot_id=existing_slot.id)
    )
    ...
    target_business = await _create_business(savepoint_session)
    target_slot = await _create_slot(
        savepoint_session, target_business.id, conflicting_start, is_booked=False
    )
...
async def test_book_returns_404_for_nonexistent_slot(
    book_savepoint_client: httpx.AsyncClient,
) -> None:
    response = await book_savepoint_client.post(
