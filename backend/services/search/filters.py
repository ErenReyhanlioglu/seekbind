...
def translate_filters_to_qdrant(filters: SearchFilters) -> Filter | None:
    ...
    if filters.max_price is not None:
        conditions.append(
            FieldCondition(key="price_min", range=Range(lte=filters.max_price))
        )
    if filters.min_price is not None:
        conditions.append(
            FieldCondition(key="price_max", range=Range(gte=filters.min_price))
        )
    ...
    if filters.category is not None:
        conditions.append(
            FieldCondition(
                key="type_normalized", match=MatchValue(value=filters.category)
            )
        )
    ...
    if filters.weekend_open_only:
        conditions.append(
            FieldCondition(key="open_weekend", match=MatchValue(value=True))
        )
        ...
        conditions.append(
            FieldCondition(
                ...
                geo_radius=GeoRadius(
                    center=GeoPoint(
                        lon=filters.near.longitude, lat=filters.near.latitude
                    ),
