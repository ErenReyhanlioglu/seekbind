...
- ✅ `feature/rag-pipeline` — RAG orkestrasyon (intent parsing + öneri üretimi — projenin asıl LLM testi). `POST /recommend` endpoint'i eklendi. Fiyat eşiği LLM tahminine değil gerçek DB verisine dayanıyor, bkz. [ADR-0014](adr/0014-price-threshold-resolution.md). Konum (`NearFilter`/"yakınımda") hâlâ ayrıştırılmıyor (geocoding altyapısı yok, bilerek kapsam dışı, bkz. [ADR-0011](adr/0011-hard-filter-vs-semantic-separation.md)). `scripts/diagnostics/smoke_test_rag.py` ile 16 senaryo (fiyat eşiği, veri penceresi dışı gün, bilinmeyen kategori, prompt injection, çoklu kategori vb.) gerçek DB/Qdrant/LLM'e karşı doğrulandı
...
- ✅ `feature/calendar-service` — bkz. [ADR-0018](adr/0018-calendar-service-booking-and-alternatives.md). `POST /book`: belirli bir slotu kullanıcıya rezerve eder, kullanıcının kendi randevularıyla (farklı işletme dahil) çakışma kontrolü yapar. Müsait değilse (dolu ya da çakışma) yapılandırılmış alternatif önerisi döner — iki kaynaktan: aynı işletmenin başka zamanı (kronolojik) + aynı kategorideki, aynı gün müsait diğer işletmeler (`weighted_rating`'e göre sıralı, NULL'lar sona — ADR-0015 deseni). Çapraz-işletme araması `search/availability.py`'deki mevcut `fetch_available_business_ids()`'i yeniden kullanıyor — yeni bir arama motoru değil, `search_providers()`'ın kendi iki fazlı deseninin küçük ölçekli bir tekrarı. Mesafe bilinçli olarak dışarıda bırakıldı, ayrı bir mini branch'e (NearFilter/"yakınımda" ile birlikte, `UserProfile`'ın referans konumu kullanılarak) saklandı. Gerçek DB'ye karşı hem elle hem `scripts/diagnostics/smoke_test_calendar.py` ile (transaction rollback edilir, kalıcı iz bırakmaz) doğrulandı
- ⬜ `feature/near-filter` — mesafe/"yakınımda" desteği. `UserProfile.latitude/longitude` (referans konum) artık var, gerçek GPS/geocoding altyapısı gerekmeden `NearFilter`'ı tetiklemek için kullanılabilir — ADR-0011'in senelerdir açık duran boşluğunu kapatabilir. Kapsam: LLM'in "yakınımda" gibi AÇIK bir ifadeyi yakalayıp `NearFilter`'ı doldurması (aksi halde mesafe hiçbir şeyi filtrelemez/sıralamaz, sadece bilgi olarak gösterilir), online işletmelerde mesafenin hiç değerlendirilmemesi
- ⬜ `test/api-integration` — `tests/integration/test_api.py`; gerçek HTTP istekleriyle uçtan uca endpoint testleri (routing + `Depends()` + Pydantic response şeması + status code). `/health` ve `/recommend` var ama otomatik (pytest) integration testi henüz yok, sadece elle (curl/httpx) doğrulandı — calendar-service'in de bitip API yüzeyi tamamlanınca, Faz 4'ün kapanışı olarak tek seferde yazılacak. Unit test coverage tarafı `test/coverage-improvements` ile ayrıca kapatıldı (%81 → %100) — bu madde artık sadece gerçek HTTP/integration testi eksikliğiyle ilgili
