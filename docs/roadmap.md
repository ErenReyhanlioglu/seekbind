...
- ⬜ `fix/rating-based-ranking` — puana göre sıralama/filtreleme mimarisi eksik, bkz. [ADR-0015](adr/0015-rating-based-ranking-gap.md) (gerçek veriyle doğrulandı: "en kötü/en düşük puanlı" sorgusu, gerçek en düşük puanlı işletmelerle hiç örtüşmedi, öneri metni de veriyle desteklenmeyen bir iddiada bulundu). Çözüm sadece prompt ile değil, `search_providers()`'ın RRF+reranker mimarisine puan sinyali eklenmesini gerektiriyor
...
- ⬜ `test/api-integration` — `tests/integration/test_api.py`; gerçek HTTP istekleriyle uçtan uca endpoint testleri (routing + `Depends()` + Pydantic response şeması + status code). `/health` ve `/recommend` var ama otomatik (pytest) integration testi henüz yok, sadece elle (curl/httpx) doğrulandı — calendar-service/tool-calling'in de bitip API yüzeyi tamamlanınca, Faz 4'ün kapanışı olarak tek seferde yazılacak. Bu arada `backend/api/routes.py`, `backend/db/qdrant.py`, `backend/db/session.py`, `backend/main.py` unit test coverage'ı hâlâ %0 (bkz. `test/db-integration`) — genel coverage %81, bu dosyalar hariç tutulduğunda çok daha yüksek
