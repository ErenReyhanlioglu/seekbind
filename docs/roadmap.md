...
- ⬜ `test/api-integration` — `tests/integration/test_api.py`; gerçek HTTP istekleriyle uçtan uca endpoint testleri (routing + `Depends()` + Pydantic response şeması + status code). `/health` ve `/recommend` var ama otomatik (pytest) integration testi henüz yok, sadece elle (curl/httpx) doğrulandı — calendar-service/tool-calling'in de bitip API yüzeyi tamamlanınca, Faz 4'ün kapanışı olarak tek seferde yazılacak. Unit test coverage tarafı `test/coverage-improvements` ile ayrıca kapatıldı (%81 → %100) — bu madde artık sadece gerçek HTTP/integration testi eksikliğiyle ilgili
